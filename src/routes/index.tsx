...
import vodacomMpesaLogo from "../assets/android-icon-48x48.png";
        ...
        <div className="mt-6 relative">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 text-center">
            ...
            <Stat
              ...
              label="Groups"
              value={String(useApp.getState().groups.length)}
            />
          </div>
        </div>
      </section>
        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar -mx-5 px-5 pb-2">
          {useApp.getState().groups.map((g) => (
            <Link
              ...
              key={g.id}
              onClick={() =>
                useApp.getState().setDraft({ participantIds: g.members.map((m) => m.id) })
              }
...
function SectionTitle({ title, link }: { title: string; link?: string | null }) {
  return (
    <div className="flex items-center justify-between mb-3">
      ...
      {link && (
        <Link to={link} className="text-xs font-medium text-brand">
          See all
        </Link>
      )}
    </div>
...
function CollectionCard({
  ...
  c: ReturnType<typeof useApp.getState>["collections"][number];
  settled?: boolean;
}) {
      ...
      <div className="mt-3">
        ...
        <div className="flex items-center justify-between mt-2 text-xs">
          ...
          <span className="text-muted-foreground">
            {paidCount}/{c.participants.length} paid
          </span>
        </div>
      </div>
    </Link>
